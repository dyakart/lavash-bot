...
class Paginator:
    ...
    def get_next(self):
        ...
        raise IndexError(f'Следующей страницы не существует. Use has_next() to check before.')

    def get_previous(self):
        if self.page > 1:
            self.page -= 1
            return self.__get_slice()
        raise IndexError(f'Предыдущей страницы не существует. Use has_previous() to check before.')
...
async def orm_create_categories(session: AsyncSession, categories: list):
    query = select(Category)
    result = await session.execute(query)
    # если нет категорий - выходим
    if result.first():
        return
    # иначе создаем все категории
